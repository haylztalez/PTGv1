...
SYS_MODULE_OBJ DRV_SPI0_Initialize(void)
{
    ...
    PLIB_SPI_PinDisable(SPI_ID_3, SPI_PIN_SLAVE_SELECT);

    /* Set up if the SPI is allowed to run while the rest of the CPU is in idle mode*/
    PLIB_SPI_StopInIdleEnable( SPI_ID_3 );

    /* Set up clock Polarity and output data phase*/
    PLIB_SPI_ClockPolaritySelect( SPI_ID_3, SPI_CLOCK_POLARITY_IDLE_HIGH );
    PLIB_SPI_OutputDataPhaseSelect( SPI_ID_3, SPI_OUTPUT_DATA_PHASE_ON_ACTIVE_TO_IDLE_CLOCK );
    ...
    PLIB_SPI_FramedCommunicationDisable( SPI_ID_3  );
            ...
            if (PLIB_SPI_ExistsAudioProtocolControl(SPI_ID_3))
            {
    ...
    /* Buffer type selection */
        ...
        if (PLIB_SPI_ExistsFIFOControl( SPI_ID_3 ))
        {
            PLIB_SPI_FIFOEnable( SPI_ID_3 );
            PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_3, SPI_FIFO_INTERRUPT_WHEN_TRANSMIT_BUFFER_IS_COMPLETELY_EMPTY);
            PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_3, SPI_FIFO_INTERRUPT_WHEN_RECEIVE_BUFFER_IS_NOT_EMPTY);
        }
    #else
        {
...
void DRV_SPI0_Tasks ( void )
{
    ...
    DRV_SPI0_ISRMasterEBM16BitTasks(&gDrvSPI0Obj);
}
...
{
    /* Call the respective task routine */
    DRV_SPI1_ISRMasterEBM16BitTasks(&gDrvSPI1Obj);
}
...
SYS_MODULE_OBJ DRV_SPI2_Initialize(void)
{
    ...
    /* Communication Width Selection */
    PLIB_SPI_CommunicationWidthSelect ( SPI_ID_2, SPI_COMMUNICATION_WIDTH_16BITS );
...
{
    /* Call the respective task routine */
    DRV_SPI2_ISRMasterEBM16BitTasks(&gDrvSPI2Obj);
}
...
int32_t DRV_SPI2_ISRErrorTasks(struct DRV_SPI_OBJ * dObj)
{
...
int32_t DRV_SPI2_BufferAddWriteRead(const void * txBuffer, void * rxBuffer, uint32_t size)
{
    ...
    do {
        ...
        {
            while (!PLIB_SPI_TransmitBufferIsFull(SPI_ID_2) && (txcounter < (size>>1)) && unitsTxed != maxUnits)
            {
                PLIB_SPI_BufferWrite16bit(SPI_ID_2, ((uint16_t*)txBuffer)[txcounter]);
                txcounter++;
                continueLoop = true;
                unitsTxed++;
            }
        ...
        while (txcounter != rxcounter)
        {
            ...
            ((uint16_t*)rxBuffer)[rxcounter] = PLIB_SPI_BufferRead16bit(SPI_ID_2);
            rxcounter++;
            continueLoop = true;
        }
        if (txcounter > rxcounter)
