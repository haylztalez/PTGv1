...
int32_t DRV_SPI0_ISRMasterEBM16BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        {
            DRV_SPI0_MasterEBMSend16BitISR(dObj);
        }
        ...
        if (bytesLeft != 0)
        {
            DRV_SPI0_MasterEBMReceive16BitISR(dObj);
...
int32_t DRV_SPI1_ISRMasterEBM16BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    volatile bool continueLoop;
    /* Disable the interrupts */
    ...
    do {
        ...
        /* Execute the sub tasks */
            ...
            (currentJob->dataLeftToTx +currentJob->dummyLeftToTx != 0)
        {
            DRV_SPI1_MasterEBMSend16BitISR(dObj);
        }
        ...
        if (bytesLeft != 0)
        {
            DRV_SPI1_MasterEBMReceive16BitISR(dObj);
            bytesLeft = currentJob->dataLeftToRx + currentJob->dummyLeftToRx;
        }
        if (bytesLeft == 0)
        {
...
int32_t DRV_SPI2_ISRMasterEBM16BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    volatile bool continueLoop;
    /* Disable the interrupts */
    ...
    do {
        ...
        if (dObj->currentJob == NULL)
        {
            ...
            /* List the new job as processing*/
            currentJob->status = DRV_SPI_BUFFER_EVENT_PROCESSING;
            if (currentJob->dataLeftToTx +currentJob->dummyLeftToTx > PLIB_SPI_RX_8BIT_FIFO_SIZE(SPI_ID_2))
            {
                PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_2, SPI_FIFO_INTERRUPT_WHEN_TRANSMIT_BUFFER_IS_1HALF_EMPTY_OR_MORE);
                PLIB_SPI_FIFOInterruptModeSelect(SPI_ID_2, SPI_FIFO_INTERRUPT_WHEN_RECEIVE_BUFFER_IS_1HALF_FULL_OR_MORE);
            }
            /* Flush out the Receive buffer */
        ...
        /* Execute the sub tasks */
            ...
            (currentJob->dataLeftToTx +currentJob->dummyLeftToTx != 0)
        {
            DRV_SPI2_MasterEBMSend16BitISR(dObj);
        }
        
        DRV_SPI2_ISRErrorTasks(dObj);
        ...
        if (bytesLeft != 0)
        {
            DRV_SPI2_MasterEBMReceive16BitISR(dObj);
            bytesLeft = currentJob->dataLeftToRx + currentJob->dummyLeftToRx;
        }
        if (bytesLeft == 0)
        {
